...
const HomeSection: React.FC<HomeSectionProps> = ({ friendChannels, friendReqs, isFriendsOpen }) => {
    ...
    return (
        <section
            className={`home-section-container ${isFriendsOpen ? 'home-section-container-mob' : ''}`}
        >
            <section className="friend-section">
                <FriendList friends={sortedFriends} setIsFriendConnection={setIsFriendConnection} />
                <div
                    className={`friend-connection-container ${isFriendConnection ? 'friend-connection-container-active' : ''}`}
                >
                    <button
                        onClick={closeFriendConnection}
                        className="connections-to-friendlist-button"
                    >
                        <svg
                    ...
                    <div className="connections-button-container">
                        <button
                            ...
                            onClick={() => setCurrComponent('addFriend')}
                            style={{
                                borderBottom: `${currComponent === 'addFriend' ? '1px solid grey' : ''}`,
                            }}
                        >
                            Add Friend
                        </button>
                        <button
                            ...
                            onClick={() => setCurrComponent('friendReq')}
                            style={{
                                borderBottom: `${currComponent === 'friendReq' ? '1px solid grey' : ''}`,
                            }}
                        >
                            Friend Requests
