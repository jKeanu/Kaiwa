...
const LoginPage: React.FC<{ isError: boolean }> = ({ isError: authenticationError }) => {
    ...
    const [loading, setLoading] = useState<{ isLoading: boolean; type: string }>({
        isLoading: false,
        type: '',
    });
    ...
    return (
        <div className={`login-container ${containerVisible ? 'visible' : ''}`}>
                ...
                <form className="login-form" onSubmit={handleLogin}>
                    <div className="input-container">
                        <input
                            ...
                            required
                            style={{
                                borderBottomColor: `${(errorMessage || forgotPassError) && '#c93a3a'}`,
                            }}
                        />
                    ...
                    <div className="input-container login-password-input-container">
                        ...
                        <button
                            ...
                            type="button"
                            disabled={loading.isLoading || authenticationError}
                            onClick={handleForgotPassword}
                        >
                            {loading.type === 'forgotPassword' ? (
            ...
            <span className="need-an-account-text">
                Need an account?{' '}
                <Link className="register-link" to="/register" aria-label="Register account">
                    Register
                </Link>
            </span>
        </div>
