...
const RegisterPage: React.FC<{ isError: boolean }> = ({ isError: authenticationError }) => {
    ...
    const handleRegister = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        if (formData.passwordConfirm !== formData.password) {
            return setErrorMessage({ type: 'password', message: 'Confirm password is incorrect.' });
        }
    ...
    return (
        <div className={`register-container ${containerVisible ? 'visible' : ''}`}>
            ...
            <div className="register-form-container">
                <form className="register-form" onSubmit={handleRegister}>
                    <div className="input-container">
                        <input
                            ...
                            style={{
                                borderBottomColor: `${
                                    errorMessage &&
                                    (errorMessage.type === 'email' || errorMessage.type === 'other')
                                        ? '#c93a3a'
                    ...
                    <div className="input-container">
                        ...
                        {errorMessage &&
                            (errorMessage.type === 'password' || errorMessage.type === 'other') && (
                                <span className="input-error-message">{errorMessage.message}</span>
                            )}
                    </div>
                    <div className="input-container">
            ...
            <span className="already-have-an-account-text">
                ...
                <Link className="log-in-link" to="/login" aria-label="Return to login">
                    Log In
                </Link>
            </span>
        </div>
