...
const ResetPassword: React.FC<{ isError: boolean }> = ({ isError: authenticationError }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (resetPasswordToken) {
            ...
            setIsLoading(true);
            try {
                const res: AxiosResponse<{ status: string }> = await resetPassword(
                    resetPasswordToken,
        ...
        <div className={`reset-password-container ${containerVisible ? 'visible' : ''}`}>
            <form className="reset-password-form" onSubmit={handleSubmit}>
                <div className="input-container">
                    ...
                    {errorMessage && (
                        <span className="input-error-message" id="input-error-message">
                            {errorMessage}
                        </span>
                    )}
                </div>
                <div className="input-container">
                    <input
                        ...
                        style={{ borderBottomColor: `${errorMessage && '#c93a3a'}` }}
                    />
                    <label htmlFor="resetPasswordConfirm" className="input-label">
                        Cofirm Password
                    </label>
                    {!errorMessage && <span className="input-highlight"></span>}
                    {errorMessage && (
