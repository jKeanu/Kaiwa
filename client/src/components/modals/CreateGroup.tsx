...
const CreateGroupModal: React.FC<CreateGroup> = ({
    currUserId,
    setIsDisabled,
    setModal,
    handleCloseButton,
    ...
    const { friendsInfo, channelsDispatch, socket, setModalVisible, modalVisible } =
        useLeftCustomContext();

    const filteredFriends = useMemo(() => {
        return [...friendsInfo]
            .filter((friends) =>
                friends.displayName.toLowerCase().includes(searchQuery.toLocaleLowerCase())
            )
    ...
    const handleGroupName = (e: React.MouseEvent<HTMLButtonElement>): void => {
        ...
        if (groupName === '') {
            setCreateGroupErr({
                err: true,
                type: 'groupName',
                message: 'Please provide a name for your channel.',
            });
        } else {
            ...
            if (axios.isAxiosError(err) && err.response) {
                if (err.response.status === 400) {
                    ...
                    setCreateGroupErr({
                        err: true,
                        type: 'createErr',
                        message: 'Too many group creation detected, please try again later.',
                    });
                } else {
                    setCreateGroupErr({
                        err: true,
    ...
    return (
        <div className={`create-group-modal-container ${modalVisible ? 'visible' : ''}`}>
            ...
            >
                <div
                    className={`create-group-modal-name-container ${field === 'memberSelect' ? 'name-container-after' : ''}`}
                >
                    <div className="modal-text">
                        Give your channel a name. You can change it later.
                    </div>
                    {createGroupErr && (
                        <div className="create-group-error">{createGroupErr.message}</div>
                    )}
                ...
                <input
                    className="create-group-search-mob"
                    placeholder="Search friends..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.currentTarget.value)}
                    id="friend-search-mob-create-group"
                />
                <div
                    className={`create-group-modal-invite-container ${field === 'memberSelect' ? 'invite-container-active' : ''}`}
                >
                    <div className="modal-text">Form a new group with your friends</div>
                    <input
                        className="create-group-search"
                        placeholder="Search friends..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.currentTarget.value)}
                        id="search-friend-create-group"
                    />
                                ...
                                <li key={index} className="friend-checkbox-container">
                                    ...
                                    >
                                        <input
                                            ...
                                            checked={members.includes(friend._id) ? true : false}
                                            type="checkbox"
                                            value={friend._id}
