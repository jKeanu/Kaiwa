...
const GroupSettingsModal: React.FC<GroupSettingsProps> = ({
    ...
    const profileImagePreview = useMemo(() => {
        return groupFormData.groupProfileImage
            ? URL.createObjectURL(groupFormData.groupProfileImage)
            : null;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (groupFormData.channelName !== channelName)
                formData.append('channelName', groupFormData.channelName);
            ...
            const res: AxiosResponse<UpdateGroupStatus> = await changeGroupSettings(
                channelId,
                formData
            );
            ...
            if (axios.isAxiosError(err)) {
                if (err.response?.status === 429) {
                    setGroupSettErr({
                        err: true,
                        message:
                            'Too many group setting change attempts were detected, please try again later.',
                    });
                    ...
                    if (errMessages.split('. ').length > 1) {
                        ...
                        setGroupSettErr({
                            err: true,
                            message: 'There was an error changing group settings.',
                        });
                    }
                } else {
                    setGroupSettErr({
                ...
                setGroupSettErr({
                    err: true,
                    message: 'There was a problem changing the group channel information.',
                });
            }
            setIsLoading(false);
            setModalDisabled(false);
    ...
    return (
        <div className={`${modalVisible ? 'visible' : ''} group-settings-modal-container`}>
            <div className="group-setting-x-button-container">
                <button
                    className="group-setting-x-button"
                    disabled={isLoading}
            ...
            <form className="group-settings-form" onSubmit={handleSubmit}>
                <div className="group-photo-input-containter">
                    <label className="group-photo-input-label" htmlFor="group-setting-photo">
                        <img
                            src={`${profileImagePreview ? profileImagePreview : groupPhoto === 'default.jpeg' ? '/img/default.jpeg' : groupPhotoUrl}`}
                            className="group-setting-img"
                            alt={channelName + ' group profile photo'}
                        />
                    ...
                    <input
                        type="file"
                        name="profileImage"
                        id="group-setting-photo"
                        onChange={handleFileChange}
                        className="group-setting-photo-input"
                    />
                </div>
                <div className="group-info-input-container">
                    ...
                    <input
                        ...
                        value={groupFormData.channelName}
                ...
                <div className="group-setting-button-container">
                    <button className="group-setting-button" type="submit" disabled={isLoading}>
                        {isLoading ? <div className="group-setting-loading"></div> : 'Change'}
                    </button>
                </div>
                {groupSettErr.err && <div className="group-sett-error">{groupSettErr.message}</div>}
            </form>
        </div>
    );
};
