...
const ProfileSettingsModal: React.FC<ProfileSettings> = ({
    currUserData,
    setModal,
    handleCloseButton,
    setIsDisabled,
    ...
    const [userInfo, setUserInfo] = useState<{
        ...
        friendTag: currUserData.friendTag,
        profileImage: null,
    });
    ...
    const { setUserData, setModalVisible, modalVisible, socket, channelNumberAndIds } =
        useLeftCustomContext();
    ...
    const handleUserInfoSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (userInfo.displayName !== currUserData.displayName)
                formData.append('displayName', userInfo.displayName);
            if (userInfo.friendTag !== currUserData.friendTag)
            ...
            if (res.data.status === 'success') {
                ...
                if (socket) {
                    socket.emit('user-profile-settings-change', {
            ...
            if (axios.isAxiosError(err) && err.response) {
                if (err.response.status === 400) {
                    ...
                    if (errMessages.split('. ').length > 1) {
                        ...
                        setUserSettErr({
                            err: true,
                            message: 'There was an error changing the user information.',
                        });
                    }
                } else if (err.response.status === 429) {
                    setUserSettErr({
                        err: true,
                        message: 'Too many user info change attempts, please try again later.',
                    });
                } else {
                    setUserSettErr({
                        err: true,
            ...
            if (axios.isAxiosError(err)) {
                if (err.response?.status === 401) {
                    ...
                    if (errMessages.split('. ').length > 1) {
                        ...
                        setUserSettErr({
                            err: true,
                            message: 'There was an error changing the user password.',
                        });
                    }
                } else if (err.response?.status === 429) {
                    setUserSettErr({
        ...
        <div className={`profile-settings-modal-container ${modalVisible ? 'visible' : ''}`}>
            ...
            <div className="profile-settings-buttons-container">
                <button
                    ...
                    disabled={isLoading}
                    style={{
                        borderBottom: `${currSetting === 'userInfo' ? '1px solid #b9b9b9' : ''}`,
                    }}
                >
                    User Information
                </button>
                <button
                    onClick={handleUserPassClick}
                    disabled={isLoading}
                    style={{
                        borderBottom: `${currSetting === 'userPassword' ? '1px solid #b9b9b9' : ''}`,
                    }}
                >
                    Change Password
                </button>
            ...
            <div className="profile-settings">
                {currSetting !== 'contactUs' ? (
                    <>
                        <form
                            className={`user-info-form ${currSetting === 'userPassword' ? 'inactive' : ''} user-settings-form`}
                            onSubmit={handleUserInfoSubmit}
                        >
                            <div className="user-photo-input-container">
                                <label htmlFor="user-setting-photo">
                                    <img
                                        src={`${profileImagePreview ? profileImagePreview : currUserData.photo === 'default.jpeg' ? '/img/default.jpeg' : currUserData.photoUrl}`}
                                        className="user-setting-photo"
                                        alt={`${currUserData.displayName} profile photo`}
                                    />
                                ...
                                <input
                                    type="file"
                                    name="profileImage"
                                    id="user-setting-photo"
                                    onChange={handleFileChange}
                                    className="user-setting-photo-input"
                                />
                            </div>
                            <div className="user-info-input-containter">
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-email"
                                >
                                    Email
                                </label>
                                <input
                                    className="user-email-input user-info-input"
                                    id="user-setting-email"
                                    disabled={true}
                                    name="email"
                                    placeholder={`${currUserData.email}`}
                                />
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-displayName"
                                >
                                    Display Name
                                </label>
                                <input
                                    className="user-display-name-input user-info-input"
                                    id="user-setting-displayName"
                                ...
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-friendTag"
                                >
                                    Friend Tag
                                </label>
                                <input
                                    className="user-friend-tag-input user-info-input"
                                    id="user-setting-friendTag"
                                    maxLength={6}
                                    value={userInfo.friendTag}
                                    name="friendTag"
                                    onChange={handleUserInfoChange}
                            ...
                            {userSettErr.err && (
                                <div className="user-sett-error">{userSettErr.message}</div>
                            )}
                        </form>
                        <form
                            className={`user-password-form ${currSetting === 'userPassword' ? 'active' : ''} user-settings-form`}
                            onSubmit={handleUserPasswordSubmit}
                        >
                            <div className="user-password-input-container">
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-currentPassword"
                                >
                                    Current Password
                                </label>
                                <input
                                    type="password"
                                    className="user-password-input user-current-password-input"
                                    onChange={handleUserPasswordChange}
                                    name="currentPassword"
                                    id="user-setting-currentPassword"
                                    value={userPassword.currentPassword}
                                    required={true}
                                />
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-password"
                                >
                                    New Passsword
                                </label>
                                <input
                                    type="password"
                                    className="user-password-input user-password-input"
                                    onChange={handleUserPasswordChange}
                                    name="password"
                                ...
                                <label
                                    className="user-setting-input-label"
                                    htmlFor="user-setting-passwordConfirm"
                                >
                                    Confirm Passsword
                                </label>
                                <input
                                    type="password"
                                    className="user-password-input user-password-confirm-input"
                                    onChange={handleUserPasswordChange}
                                    name="passwordConfirm"
                                    id="user-setting-passwordConfirm"
                            ...
                            <div className="user-password-button-container user-settings-button-container">
                                <button
                                    className="user-password-submit user-setting-submit"
                                    type="submit"
                                    disabled={isLoading}
                                >
                                    {isLoading ? (
