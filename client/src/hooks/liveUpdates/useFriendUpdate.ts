...
const useFriendUpdate = (
    ...
    useEffect(() => {
        if (socket) {
            const handleRequestAccepted = (data: FriendRequestAccepted) => {
                const newFriendInfo = [...sentReqs].find(
                    (sentReq) => sentReq.friend._id === data.newFriendId
                );
                if (newFriendInfo) {
                    ...
                    setSentReqs((prevSentReqs) => {
                        return [...prevSentReqs].filter(
                            (sentReqs) => sentReqs.friend._id !== data.newFriendId
                        );
                    });
                }
            };
    ...
    useEffect(() => {
        if (socket) {
            const handleRequestDeclined = (data: { userId: string }): void => {
                setSentReqs((prevSentReqs) =>
                    [...prevSentReqs].filter((sentReq) => sentReq.friend._id !== data.userId)
                );
            };
    ...
    useEffect(() => {
        if (socket) {
            const handleFriendDeletion = (data: {
                ...
                if (location.pathname === `/@me/channels/${data.channelNumber}`) {
                    ...
                    setFriendChannels((prevFriendChannels) =>
                        [...prevFriendChannels].filter(
                            (channel) => channel.channel._id !== data.channelId
                        )
                    );
                }
            };
