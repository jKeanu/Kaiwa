...
const useGroupUpdate = (
    ...
    channelsDispatch: (value: ChannelAction) => void,
    ...
    useEffect(() => {
        if (socket) {
            const handleGroupDeletion = (data: {
    ...
    useEffect(() => {
        if (socket) {
            const handleNewLeader = (data: {
                channelNumber: number;
                newLeaderId: string;
            }): void => {
                safeMutate(
                    `api/v1/channels/${data.channelNumber}`,
                    groupLeaderUpdate(data.newLeaderId)
                );
            };
