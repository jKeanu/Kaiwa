...
function channelReducer(state: Channel[] | [], action: ChannelAction) {
    switch (action.type) {
        ...
        case ActionType.Seen: {
            const channels = [...state].map((channel) => {
                if (
                    ...
                    !channel.seen.includes(action.payload.currUserId)
                ) {
        ...
        case ActionType.NewMessage: {
            const { channelNumber, channelId, seen, newTime, newFormattedTime } =
                action.payload.newMessageInfo;
        ...
        case ActionType.NewMember: {
            const updateChannels = [...state];
            const currChannel = updateChannels.find(
                (channel) => channel.channelNumber === action.payload.channelNumber
            );
            if (currChannel) {
