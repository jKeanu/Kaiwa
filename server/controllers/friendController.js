...
const findFriendshipStatus = (currentUserId, userCheck, status) =>
    userCheck.friends.find(
        (friend) => friend.friend.equals(currentUserId) && friend.status === status
    );
...
export const addFriend = catchAsync(async (req, res, next) => {
    ...
    try {
        if (!friendTag || !displayName) {
            ...
            return next(
                new AppError('Please provide both friend tag and the name of the user.', 400)
            );
        }
        ...
        if (findFriendshipStatus(req.user._id, addUser, 'Pending')) {
            await session.abortTransaction();
            return next(
                new AppError(`You have already sent a friend request to ${displayName}.`, 409)
            );
        }
        ...
        if (findFriendshipStatus(req.user._id, addUser, 'Sent')) {
            await session.abortTransaction();
            return next(
                new AppError(`${displayName} have already sent you a friend request.`, 409)
            );
        }
        ...
        const currUserData = await User.findByIdAndUpdate(
            req.user._id,
            {
                $push: {
                    friends: {
        ...
        const newPendingRequestDetails = addUser.friends.find(
            (friend) => friend.friend === req.user._id
        );
        await User.populate(newPendingRequestDetails, {
            path: 'friend',
            select: 'displayName FriendTag status photo',
            options: { session },
        });
        //Sent request details
        const newSentRequestDetails = currUserData.friends.find(
            (friend) => friend.friend.toString() === addUser._id.toString()
        );
        await User.populate(newSentRequestDetails, {
            path: 'friend',
            select: 'displayName FriendTag status photo',
            options: { session },
        });
    ...
    const attemptOperation = async () => {
        ...
        try {
            ...
            if (findFriendshipStatus(req.user._id, acceptUser, 'Friend')) {
                await session.abortTransaction();
                return next(
                    new AppError(`You're already friends with ${acceptUser.displayName}.`, 409)
                );
            }
            ...
            await User.findOneAndUpdate(
                { _id: req.user._id, 'friends.friend': acceptUser._id },
                {
                    $set: {
...
export const getUserFriends = catchAsync(async (req, res, next) => {
    const user = await User.findById(req.user._id).populate({
        path: 'friends.friend',
        select: 'displayName photo friendTag',
    });
    if (!user) {
