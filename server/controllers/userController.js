...
export const updateUser = catchAsync(async (req, res, next) => {
    ...
    const updatedUserObject = await User.findByIdAndUpdate(req.user.id, sanitizedBody, {
        new: true,
        runValidators: true,
    })
...
export const getMe = catchAsync(async (req, res) => {
    ...
    const currentUserObject = await User.findById(req.user._id)
        .select('-__v')
        .populate({ path: 'friends.friend', select: 'displayName friendTag photo status' })
        .populate({
            path: 'friends.channel',
            select: 'channelNumber lastMessage formattedLastMessage channelType seen',
        })
        .populate({
            path: 'groups',
            select: 'channelNumber lastMessage channelName photo formattedLastMessage channelType seen',
        })
        .lean();
