...
export default (httpServer) => {
    ...
    io.use((socket, next) => {
        ...
        if (!cookieArray['accessToken']) return next(new Error('JWT token missing in cookies'));

        verifyToken(cookieArray['accessToken'], process.env.ACCESS_TOKEN_SECRET)
            .then((decoded) => {
                if (!decoded || !decoded.userId) return next(new Error('Invalid access token.'));
    ...
    io.on('connection', async (socket) => {
        ...
        async function joinRoom(roomId) {
            try {
                ...
                errLogger.error(`Join room error. ROOM: ${roomId}`, {
                    user: verifiedCurrentUserId,
                    error: err instanceof Error ? err.stack : err,
                });
            }
        }
        ...
        async function leaveRoom(roomId) {
            try {
                ...
                errLogger.error(`Leave room error. ROOM: ${roomId}`, {
                    user: verifiedCurrentUserId,
                    error: err instanceof Error ? err.stack : err,
                });
            }
        ...
        //JOIN ROOM
        socket.on('personal_live_update', async () => {
            await joinRoom(`user-${verifiedCurrentUserId}`);
        });

        socket.on('join_channel_room', async (channelNumber) => {
            await joinRoom(channelNumber);
        });

        socket.on('join_channel_verify_message', async (channelNumber) => {
            await joinRoom(`channel-${channelNumber}-verify-message-${verifiedCurrentUserId}`);
        });

        socket.on('channel_live_updates', async (channelIds) => {
            await Promise.all(
        ...
        //Leave Room
        socket.on('leave_personal_live_update', async () => {
            await leaveRoom(`user-${verifiedCurrentUserId}`);
        });

        socket.on('leave_channel_room', async (channelNumber) => {
            await leaveRoom(channelNumber);
        });

        socket.on('leave_channel_live_updates', async (channelIds) => {
            await Promise.all(channelIds.map((channelId) => leaveRoom(`channel-${channelId}`)));
        });

        socket.on('leave_channel_verify_message', async (channelNumber) => {
            await leaveRoom(`channel-${channelNumber}-verify-message-${verifiedCurrentUserId}`);
        });
            ...
            try {
                ...
                const updateChannel = await Channel.findByIdAndUpdate(
                    { _id: data.channel },
                    {
                        ...
                        formattedLastMessage: data.formattedDate,
                ...
                io.to(`channel-${data.channelNumber}-verify-message-${verifiedCurrentUserId}`).emit(
                    'message_verified',
                ...
                io.to(`channel-${data.channel}`).emit('channel_lastmsg_update', {
                    ...
                    newTime: data.time,
                    newFormattedTime: data.formattedDate,
                    message: messageInfo,
                    channelType: data.channelType,
                });
                socket.to(`${data.channelNumber}`).emit('receive_message', messageInfo);
            } catch (err) {
                errLogger.error('Send Message Error', {
                    error: err instanceof Error ? err.stack : err,
                });
        ...
        socket.on('user_invite_success', async (data) => {
            ...
            session.startTransaction();
            try {
                const userObject = await User.findById(data.inviteUser)
                    .select('displayName friendTag _id photo status')
                    .session(session)
                    .lean();
                userObject.photoUrl = `${cloudfrontDomainName}/${userObject.photo}`;
                const currChannelObject = await Channel.findById(data.channelId)
                    .select(
                        'photo channelNumber _id channelName channelType lastMessage id formattedLastMessage seen'
                    )
                    .session(session)
                    .lean();
                await session.commitTransaction();
                ...
                io.to(`channel-${data.channelId}`).emit(`channel_member_update`, {
                    user: userObject,
                    channelNumber: currChannelObject.channelNumber,
                    newTime: data.newTime,
                    type: 'Joined',
                });
                socket.to(`user-${userObject._id}`).emit('invited_to_group', currChannelObject);
            } catch (err) {
                errLogger.error('User Invite Success Live Update Error', {
                    error: err instanceof Error ? err.stack : err,
                });
                await session.abortTransaction();
            } finally {
        ...
        socket.on('leave_group', async (data) => {
            try {
                const user = await User.findById(verifiedCurrentUserId).select(
                    'displayName friendTag_id photo status'
                );
                ...
                errLogger.error('Leave Group Live Update Error', {
                    error: err instanceof Error ? err.stack : err,
                });
            }
        });
        ...
        socket.on('continue_message', async (data) => {
            ...
            const attemptOperation = async () => {
                try {
                    ...
                    const updateChannel = await Channel.findByIdAndUpdate(
                        { _id: data.channel },
                        { lastMessage: data.newTime, seen: [`${verifiedCurrentUserId}`] },
                        { new: true }
                    );
                    ...
                    io.to(
                        `channel-${data.channelNumber}-verify-message-${verifiedCurrentUserId}`
                    ).emit('message_verified', { sentContent: data.content });
                    socket.to(data.channelNumber).emit('receive_message', messageInfo);
                    io.to(`channel-${updatedMessage.channel}`).emit(`channel_lastmsg_update`, {
                        channelId: updatedMessage.channel,
                        channelNumber: data.channelNumber,
                        seen: updateChannel.seen,
                        channelType: data.channelType,
                        newTime: updatedMessage.time,
                        message: messageInfo,
                    });
                } catch (err) {
                    if (retries > 0) {
                    ...
                    errLogger.error('Continue Message Error', {
                        error: err instanceof Error ? err.stack : err,
                    });
                }
            };
        ...
        socket.on('new_message_seen', async (data) => {
            try {
                ...
                errLogger.error('New Message Seen Error', {
                    error: err instanceof Error ? err.stack : err,
                });
            }
        });
        ...
        socket.on('friend_request_sent', (data) => {
            socket
                .to(`user-${data.requestedUserId}`)
                .emit('receive-friend-request', data.requestDetails);
        });
        ...
        socket.on('accepted_pending_friend_request', (data) => {
            socket.to(`user-${data.pendingUserId}`).emit(`friend_request_accepted`, {
                newFriendId: data.newFriendId,
                newChannelInfo: data.newChannelInfo,
            });
        });
        ...
        socket.on('declined_pending_friend_request', (data) => {
            socket
                .to(`user-${data.declinedUser}`)
                .emit('friend_request_declined', { userId: data.userId });
        });
        ...
        socket.on('group_channel_leader_change', (data) => {
            socket.to(`user-${data.memberId}`).emit('new_group_leader', {
                channelNumber: data.channelNumber,
                newLeaderId: data.memberId,
            });
        });
